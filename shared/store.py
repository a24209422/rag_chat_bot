...
#   Chroma 的價值要到十萬塊以上才顯現——它解決的是這裡還沒遇到的問題。
#
#   ⚠ 更正（2026-09-13）：這段原本寫的理由是「Chroma 的 metadata 只吃
#     str/int/float/bool，不支援 list，多值的 facets 進去要攤平成
#     city_台北=True 這種布林欄」。那已經不成立——Chroma 現在支援陣列型
#     metadata，並提供 $contains / $not_contains，city: ["台北","新竹"]
#     可以原樣存。留著過期的理由會變成一個看起來很有說服力的藉口。
#
#   換過去現在真正要付的代價是三項：
#     · 空陣列不被允許。而 derive() 是刻意留空的（抓不到代表「資料沒寫」
#       而不是「否」），要改成整個 key 省略；match() 的交集語意要翻成
#       $or 串 $contains，test_facets.py 跟著重寫。
#     · 全文檢索是過濾不是排序。$contains / $regex 沒有 BM25 分數，
#       想做混合檢索還是得自己維護一份詞彙索引做融合。
#     · 距離度量會變（預設 L2，不是餘弦），兩邊量出來的 min_score 會全部
#       失效。所以真要換，順序是「先有評測集，再換」。
#
