...
> ⚠ 第 2 層踩過一次坑：**原樣重抽沒有用**。最早的版本就是「再抽一次」，實測 12 次有
> 7 次抽到同樣那句話——temperature 0.2 之下，只要 context 把模型推向那句話，重抽就是
> 在同一個眾數上打轉。有效的是**換掉輸入**：汙染源是上一輪那段長對話，拿掉它就好
> （不帶 history 的測試從來沒失敗過）。
...
實測 `南部呢？` 這種省略式跟隨問句，模型 **3/3 決定性地拒絕**，而同樣的【資料】換成完整
問句就 0/4 失敗：
...
試過三種重問寫法都救不回來（原樣、補成完整問句、改成「請列出【資料】裡的所有職缺」），
各 5 次全失敗。**所以第 2 層對這個 case 無效，真正救場的是第 3 層。**

這也是為什麼第 3 層不能省：它不是「以防萬一」，是有些情況下唯一有效的那一層。
