...
問「有哪些台北的職缺？」。來源列比模型的字先到——檢索比生成快得多，所以泡泡還停在
「思考中…」的時候，底下已經寫著檢索到幾筆了。吐完字展開來源列：**模型的散文列 15 筆，
來源列 20 筆**，後者才是完整的。理由見
[完整的清單由程式給，不是模型](#完整的清單由程式給不是模型)。
...
https://github.com/user-attachments/assets/99d84807-601c-4ef7-9960-9af4ed2421a4

同一句「推薦一家餐廳」問兩邊。雲端那次幾乎是瞬間回完的——0 筆就短路，連生成 API 都
沒打；地端撈得到 5 筆，只能等模型自己把話講出來。
...
| 南部有哪些職缺？ | `city=嘉義,台南,高雄,屏東` | 1 個 |
| 中部有哪些職缺？ | `city=苗栗,台中,彰化,南投,雲林` | **0 個**（語料真的沒有）|
...
「南部」不是資料裡的屬性，是**問句裡的說法**。所以 `REGION` 只在 `parse_query` 展開成
城市——職缺的 facets 不多一個欄位，`derive()` 與 `match()` 一行都不用改。

沒有它的時候，問「南部有哪些職缺」抽不出任何條件 → 退回純向量檢索 → 撈回
JETGO-CONTENT-01、Kneron-01、T504.4 這些台北新竹的職缺 → 模型誠實回「資料裡沒有」。
**錯的是檢索，不是生成**——這個區分決定了要修哪裡（不要去動 `SYSTEM`）。

`CITY` 因此列得比語料多（目前只出現台北、新北、新竹、台南四個）。多列是安全的：比對
方式是「這個詞有沒有出現在文字裡」，沒出現的城市不會憑空冒出來；而「南部」要能表達
完整的意思，就不能只列語料裡剛好有的那幾個。

### 篩選過了要講給模型聽

展開之後檢索就對了，但**模型還是回「資料裡沒有」**——實測 6 次全錯。原因很單純：
Qwen2.5-3B 不知道台南屬於南部。

所以 prompt 多一句，把展開結果直接寫給它看：

```
（【資料】已依「地點＝嘉義／台南／高雄／屏東」篩選完畢，列出的就是全部符合的職缺。）
```

實測 6 次全對。這不是「叫模型聽話」的咒語，是**補一塊它沒有的知識**——差別在於前者靠
運氣，後者可以解釋為什麼會有效。

沒有過濾條件時不加這句：純向量檢索撈回來的是「最像的前 k 個」，不是「全部符合的」，
講了會是謊話。

> ⚠ 中間試過一個較弱的版本（只說「已篩選完畢」，不寫展開結果）。模型改口說「資料裡
> 沒有**符合南部地點的**職缺」——答案還是錯的，而且當時**躲過了矛盾偵測**。
> 偵測器後來補強了（見下面），但這筆留著：**能繞過偵測器的修法比沒修更危險**。
...
| `token` | 一段文字 | 模型每吐一段 |
| `done` | `usage`、更新後的 `history`、`contradiction` | 正常結束 |
...
地端 Qwen2.5-3B 有一個偶發失態：【資料】裡明明就有那個職缺，它還是回「資料裡沒有」。
同一題跑 46 次，**檢索 46 次都對**、生成錯 5 次；換一段上下文之後最高到 12 次錯 7 次
——失敗率高度取決於上一輪答案的確切文字。

這個矛盾**偵測得到**（`shared/chat_bot.py` 的 `contradicts`），判斷分兩個條件：

1. 回答以「資料裡沒有」那一族的措辭開頭
2. **回答從頭到尾沒提到任何一個撈回來的代號**

第 2 個條件才是關鍵，它把「真的在拒絕」跟「先否定再回答」分開，而且剛好搭在既有的
契約上——`SYSTEM` 本來就要求提到職缺一定要附代號。

> ⚠ 第一版只認字面相等（整段就是「資料裡沒有。」）。上線之後就被冗長版本繞過去了：
> 「資料裡沒有**符合南部地點要求的**職缺。」——INT-01 就在台南，這句話是錯的，但偵測
> 不到 → 不重問 → 使用者看到一句假話。**能被繞過的偵測器比沒有更危險**，因為它讓人
> 以為已經有防護。

偵測到之後分三層處理：

1. **押著不吐**。串流時開頭幾個字先押著，直到確定它不是那一句為止——最多押 6 個字，
   使用者感覺不出來，而一旦分歧就整批補吐。所以畫面不會先閃過一句錯的再被換掉。
   這裡有個成立的不變式：矛盾為真代表整段都被押著，所以「重抽」永遠不會發生在已經
   送出東西之後。
2. **換掉輸入再問一次**。優先把省略式問句補成完整問句（`南部呢？` → `南部有哪些職缺？`），
   history 留著；補不出來的（`那薪水呢？` 抽不到任何可比對的詞）就退回「丟掉 history
   只留這一輪」。見 `_retry_send`。
3. **還是矛盾就明講**：`ChatResponse.contradiction` 與 SSE 的 `done` 事件帶這個旗標，
   React 與 Streamlit **兩邊**都顯示一行「模型說資料裡沒有，但檢索到了 N 筆——以下面
   的來源為準」。

> ⚠ 第 2 層踩過兩次坑。
>
> 一、**原樣重抽沒有用**。最早的版本就是「再抽一次」，實測 12 次有 7 次抽到同樣那句
> 話——temperature 0.2 之下，只要 context 把模型推向那句話，重抽就是在同一個眾數上
> 打轉。**要換掉輸入才換得到不同的答案。**
>
> 二、換成「丟掉 history」之後，對省略式問句仍然無效（4/4 失敗）。真正有效的是補句，
> 見下一節——而我一度以為那條路也走不通，理由是測錯了東西。

第 3 層才是真正的保險，而它的理由跟整個專案的立場一致：**來源列是完整精確的清單，
模型的散文只當摘要看**。兩者打架時該信誰，[api/schemas.py](api/schemas.py) 開頭早就寫好了。

### 省略式問句：唯一確定的變因是問句怎麼寫

實測 `南部呢？` 這種省略式跟隨問句，模型 **決定性地拒絕**（3/3、4/4、6/6），而同樣的
【資料】換成完整問句就 0/4 失敗：
...
檢索、過濾、附註全都正確——INT-01 每一次都撈得到。**唯一的變因是送進生成的那一行
【問題】怎麼寫。**而且它跟 history 無關：完全不帶 history 也一樣失敗。

所以第 2 層改成「把問句補齊再問一次」，用的是 `facets.terms_in()` 抓回來的**使用者
原字**：

```
南部呢？  →  terms_in → ["南部"]  →  as_question → 南部有哪些職缺？
```

帶著同一段 history、同一份【資料】跑 `ask_stream`，**6/6 全對**（每次都觸發重問，所以
這種問句固定要付兩次生成）。

> ⚠ 一定要用原字。`parse_query` 回的是正規化後的值，補出來會變成「嘉義、台南、高雄、
> 屏東有哪些職缺？」——那是另一句話，沒有被驗證過。量過 6/6 對的是「南部有哪些職缺？」。

**我一度以為這條路走不通**，因為試了三種寫法各 5 次全失敗。但回頭看，那三種沒有一個是
真正的補句：一個是原樣沒改，一個是在原句**後面**接字（`南部呢？有哪些職缺？`，保留了
會壞事的開頭），一個是丟掉「南部」換成通用指令。**測錯了東西，卻下了「這條路無效」的
結論**——記在這裡。

抽不到任何可比對的詞時（`那薪水呢？`）補不出問句，退回「丟掉 history」。那條路的證據
比較弱，但聊勝於無。

第 3 層仍然不能省：補句只在抽得到詞的時候有效，而且補完也不保證模型會照做。

⚠ 偵測仍然不是正確性保證。它認得的是「拒絕且沒引用任何代號」；模型若給出一個**內容
錯誤但有引用代號**的回答（實測 3B 會把代號張冠李戴），這一層抓不到。判斷任意錯答案
是做不到的事，不要假裝做得到。
