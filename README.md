...
> 沒有**符合南部地點的**職缺」——躲過了下面那個矛盾偵測，答案還是錯的。
> **能繞過偵測器的修法比沒修更危險**，所以把這筆留在這裡。
...
這個矛盾**偵測得到**（`shared/chat_bot.py` 的 `contradicts`：hits 非空，回答卻只有那一句），
所以處理分三層：
...
3. **還是矛盾就明講**：`ChatResponse.contradiction` 與 SSE 的 `done` 事件帶這個旗標，
   前端顯示一行「模型說資料裡沒有，但檢索到了 N 筆——以下面的來源為準」。
...
⚠ 偵測只認得「整段就是那一句」。模型如果說「資料裡沒有符合南部地點的職缺」就抓不到
——判斷任意的錯答案是做不到的事，不要假裝做得到。這一層是安全網，不是正確性保證。
