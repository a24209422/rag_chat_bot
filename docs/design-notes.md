...
這份記錄「為什麼這樣切」與「踩到過什麼」。安裝、啟動、API 用法在
[README.md](../README.md)。
...
`tools/probe_threshold.py` 會列出「資料裡有的 / 沒有的 / 跟隨問句」三組問題的相似度分佈，算出兩群之間的間隙，判斷這個間隙可不可信，再決定要給門檻還是給下限。
...
問答邏輯包成 HTTP 服務、Streamlit 改成打它，好處是實質的：後端可以獨立測試、可以被別的
東西呼叫（`curl`、React、別的服務）、可以部署在另一台機器；UI 掛掉也不會拖垮索引。

兩個聊天端點吃同一組參數（`question`、`history`、`side`、`k`），也走同一份 `ChatBot`
——差別只在回應怎麼送。兩個前端因此沒有重複的問答邏輯，差別只在畫面。

`code` 是獨立欄位而不是埋在 `reply` 的散文裡，理由見
...
**重建 `jobs.json` 之後這些數字會變，要跟這份文件一起更新。**
